...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      dpr: dpr ?? (typeof window !== 'undefined' ? Math.min(window.devicePixelRatio || 1, 2) : 1), // Cap DPR at 2 for performance
      alpha: true,
      antialias: true
    });
...
`;
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      // Use window event, so calculate relative to canvas rect safely
    ...
    const loop = (t: number) => {
      ...
      // Optimization: Only render if not paused and visible
